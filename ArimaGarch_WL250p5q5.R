...
write.table(forecasts_new, file="forecasts_WL250p5q5.csv", sep = ",",
            row.names = F, col.names = F, quote = F)

spArimaGarch = as.xts(
    read.zoo(file = "forecasts_WL250p5q5.csv", format = "%Y-%m-%d", header = F, sep = ",")
)
...
trellis.device(device = "png", filename = "ReturnsWL250p5q5.png")
print(returns_plot)
dev.off()
