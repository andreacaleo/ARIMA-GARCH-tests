...
for (d in 0:foreLength) {
    ...
    for (p in 0:5) for (q in 0:5) { 
        ...
        if( !is.logical( arimaFit ) ) {
            current.aic <- BIC(arimaFit)   # uses BIC rather than AIC
...
write.table(forecasts_new, file="forecasts_WL250p5q5BIC.csv", sep = ",",
            row.names = F, col.names = F, quote = F)

spArimaGarch = as.xts(
    read.zoo(file = "forecasts_WL250p5q5BIC.csv", format = "%Y-%m-%d", header = F, sep = ",")
)
...
trellis.device(device = "png", filename = "Returns_WL250p5q5_BIC.png")
print(returns_plot)
dev.off()
